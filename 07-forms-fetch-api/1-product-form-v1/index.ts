...
export default class ProductForm {
  ...
  async render(): Promise<HTMLElement | null> {
    ...
    this.imagesContainer = this.element.querySelector('[data-element="imageListContainer"] ul') as HTMLDivElement;
      ...
      <div class="product-form">
        <form data-element="productForm" class="form-grid">
          ...
          <div class="form-group form-group__wide" data-element="sortable-list-container">
            ...
            ${this.imageListContainerTemplate({name: 'uploadImage', images: this.product?.images})}
          </div>
  ...
  private imageListContainerTemplate(
    { name = '', buttonLabel = '', images = [] }: {
      name?: string;
      buttonLabel?: string;
      images?: ProductImage[];
    } = {}
    ...
    if (images?.length) {
      images.forEach(image  => {
        ...
        options.push(this.imageTemplate(productImage));
      })
    };
    return `
      <div data-element="imageListContainer">
        <ul class="sortable-list">${options.join('')}</ul>
        <button type="button" name="${name}" class="button-primary-outline"><span>${buttonLabel || 'Загрузить'}</span></button>
      </div>
    `;
  }
